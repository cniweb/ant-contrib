...
public class Design {
			...
			for (int i = 0; i < depends.length; i++) {
				...
				if (dependsPackage == null) {
					throw new RuntimeException("package name=" + p.getName() + " did not\n" + "have " + depends[i]
							+ " listed before it.  circularDesign is off\n" + "so package=" + p.getName()
							+ " must be moved up in the xml file");
				}
			}
		}
	...
	public void verifyDependencyOk(String className) {
		...
		// check if this is an needdeclarations="false" package, if so, the
		// dependency is ok if it
		// is not declared
		...
		// each time. for now though, just get it working and do the
		// basic(optimize later if needed)
		for (int i = 0; i < depends.length; i++) {
			Depends d = depends[i];
			...
			if (isClassInPackage(className, temp)) {
				temp.setUsed(true); // set package to used since we are
									// depending on it(could be external package
									// like junit)
	...
	public boolean isClassInPackage(String className, Package p) {
		...
		if (p.isIncludeSubpackages()) {
			if (className.startsWith(p.getPackage()))
				return true;
		} else { // if not including subpackages, the it must be the exact
					// package.
	...
	void checkClass(String dependsOn) {
		...
		// className with no package like Object, String or Class, so here we
		// try to
		...
		// sometimes instead of passing java.lang.String or java.lang.Object,
		// the bcel
	...
	public static String getErrorMessage(String className, String dependsOnClass) {
		String s = "\nYou are violating your own design...." + "\nClass = " + className + " depends on\nClass = "
				...
				+ VerifyDesignDelegate.getPackageName(className) + " is not defined to depend on" + "\nPackage="
				+ VerifyDesignDelegate.getPackageName(dependsOnClass) + "\nChange the code or the design";
		return s;
	}
