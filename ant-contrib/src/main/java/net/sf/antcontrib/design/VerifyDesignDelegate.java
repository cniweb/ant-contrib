...
public class VerifyDesignDelegate implements Log {
	...
	public void execute() {
		if (!designFile.exists() || designFile.isDirectory())
			throw new BuildException(
					"design attribute in verifydesign element specified an invalid file=" + designFile);
		...
		try {
			...
			// this is because you end up with false unused errors if you don't
			// do this.
			...
			if (e.getException() != null && e.getException() instanceof RuntimeException)
				...
				throw new BuildException("\nProblem parsing design file='" + designFile + "'.  \nline="
						+ pe.getLineNumber() + " column=" + pe.getColumnNumber() + " Reason:\n" + e.getMessage() + "\n",
						e);
			}
	...
	private void verifyPartOfPath(String fileName, File file, Design d) throws IOException {
		if (fileName.endsWith(".jar") || fileName.endsWith(".war")) {
			...
			throw new BuildException(
					"Only directories, jars, wars, and class files can be supplied to verify design, not file="
