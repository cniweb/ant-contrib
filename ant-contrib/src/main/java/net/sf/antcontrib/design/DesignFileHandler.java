...
class DesignFileHandler implements ContentHandler {
	 ...
	 * @param systemId
	 *            The system identifier provided in the XML document. Will not
	 *            be <code>null</code>.
	 */
	public InputSource resolveEntity(String publicId, String systemId) {
		log.log("publicId=" + publicId + " systemId=" + systemId, Project.MSG_VERBOSE);
		return null;
	}
		...
		try {
			...
			if (name.equals(DESIGN)) {
				...
				throw new SAXParseException(
						"Error in file=" + file.getAbsolutePath() + ", Unexpected element \"" + name + "\"", locator);
			}
			stack.push(o);
	...
	private Design handleDesign(Attributes attrs) throws SAXParseException {
		if (attrs.getLength() > 0)
			throw new SAXParseException(
					"Error in file=" + file.getAbsolutePath() + ", no attributes allowed for " + DESIGN + " element",
					locator);
		else if (stack.size() > 0)
			throw new SAXParseException("Error in file=" + file.getAbsolutePath() + ", " + DESIGN
					+ " cannot be a subelement of " + stack.pop(), locator);
		else if (attrs.getLength() > 0)
			throw new SAXParseException(
					"Error in file=" + file.getAbsolutePath() + ", " + DESIGN + " element can't have any attributes",
					locator);
	...
	private Package handlePackage(Attributes attrs) throws SAXParseException {
		if (stack.size() <= 0 || !(stack.peek() instanceof Design))
			throw new SAXParseException("Error in file=" + file.getAbsolutePath() + ", " + PACKAGE
					+ " element must be nested in a " + DESIGN + " element", locator);
		...
		for (int i = 0; i < len; i++) {
			...
			if ("name".equals(attrName))
				...
				throw new SAXParseException("Error in file=" + file.getAbsolutePath() + "\n'" + attrName
						+ "' attribute is an invalid attribute for the package element", locator);
		}
		...
		if (name == null)
			throw new SAXParseException(
					"Error in file=" + file.getAbsolutePath() + ", package element must contain the 'name' attribute",
					locator);
	...
	private Depends handleDepends(Attributes attrs) throws SAXParseException {
		if (stack.size() <= 0 || !(stack.peek() instanceof Package))
			throw new SAXParseException("Error in file=" + file.getAbsolutePath() + ", " + DEPENDS
					+ " element must be nested in a " + PACKAGE + " element", locator);
		else if (attrs.getLength() > 0)
			throw new SAXParseException(
					"Error in file=" + file.getAbsolutePath() + ", " + DEPENDS + " element can't have any attributes",
					locator);
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		try {
			...
			if (o instanceof Package) {
				...
				if (tmp != null)
					throw new SAXParseException("Error in file=" + file.getAbsolutePath() + "\nname attribute on "
							+ PACKAGE + " element has the same\n" + "name as another package.  name=\"" + p.getName()
							+ "\" is used twice or more", locator);
