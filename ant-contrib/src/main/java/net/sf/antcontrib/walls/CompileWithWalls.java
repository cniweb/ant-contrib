	...
	public void execute() throws BuildException {
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < srcDirs2.size(); i++) {
				...
				log(toCompile.getPackage() + ": sourceDir[" + i + "]=" + srcDir + " destDir=" + buildSpace,
						Project.MSG_VERBOSE);
			...
			// clone
			// instead of just reusing the other Javac....this means added
			// params in
			// future releases will be missed unless this task is kept up to
			// date.
			// need to convert to reflection later so we don't need to keep this
			// up to
			// date.
			...
			// bootclasspathref was already copied above(see javac and you will
			// understand)
			...
			// copy class files to javac's destDir where the user wants the
			// class files
