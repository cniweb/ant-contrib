/*
 * Copyright (c) 2001-2004 Ant-Contrib project. All rights reserved. Licensed under the Apache License, Version 2.0 (the
...
/*
 * Created on Aug 24, 2003 To change the template for this generated file go to Window&gt;Preferences&gt;Java&gt;Code
...
public class CompileWithWalls extends Task {
  ...
  public void execute() throws BuildException {
    if (cachedIOException != null)
      ...
      throw new BuildException("intermediaryBuildDir attribute must be specified on the compilewithwalls element",
          getLocation());
    else if (javac == null)
      ...
      throw new BuildException("compilewithwalls task only supports one nested walls element or one walls attribute",
          getLocation());
  ...
  /**
   * Move java or class files to temp files or moves the temp files back to java or class files. This must be done
   * because javac is too nice and sticks already compiled classes and ones depended on in the classpath destroying the
   * compile time wall. This way, we can keep the wall up.
   * 
   * @param srcDir
   *          Directory to copy files from/to(Usually the java files dir or the class files dir)
   * @param fileset
   *          The fileset of files to include in the move.
   * @param moveToTempFile
   *          true if moving src files to temp files, false if moving temp files back to src files.
   * @param isJavaFiles
   *          true if we are moving .java files, false if we are moving .class files.
   */
