...
public class Walls {
	...
	public void addConfiguredPackage(Package p) {
		...
		if (!pack.endsWith(".*") && !pack.endsWith(".**"))
			p.setFaultReason("The package='" + pack + "' must end with " + ".* or .** such as biz.xsoftware.* or "
					+ "biz.xsoftware.**");
		...
		for (int i = 0; i < depends.length; i++) {
			...
			if (dependsPackage == null) {
				p.setFaultReason("package name=" + p.getName() + " did not have " + depends[i]
	...
	public Iterator getPackagesToCompile() {
		// must return the list, as we need to process in order, so
		// unfortunately
