 ...
 *
 * Based on org.apache.org.apache.tools.ant.taskdefs.UpToDate
 *
 * @author peter reilly
 */
...
public class OutOfDate extends Task implements Condition {
	...
	public static class CollectionEnum extends EnumeratedAttribute {
		...
		public static final int SOURCES = 0, TARGETS = 1, ALLSOURCES = 2, ALLTARGETS = 3;
	...
	/**
	 * Set the collection attribute, controls what is returned by the iterator
	 * method.
	 * <dl>
	 * <li>"sources" the sources that are newer than the corresponding targets.
	 * </li>
	 * <li>"targets" the targets that are older or not present than the
	 * corresponding sources.</li>
	...
	/**
	 * The property to set if any of the target files are outofdate with regard
	 ...
	 * 
	 * @param force
	 *            true means that outofdate is always set, default false
	 */
	...
	/**
	 * Evaluate (all) target and source file(s) to see if the target(s) is/are
	 * outoutdate.
	 * 
	...
	public boolean eval() {
			...
			for (int i = 0; i < spaths.length; ++i) {
				...
				if (mapped != null) {
					for (int j = 0; j < mapped.length; ++j) {
						if (outOfDate(new File(spaths[i]), fileUtils.resolveFile(baseDir, mapped[j]))) {
							ret = true;
						}
					}
				}
		...
		if (allTargets != null) {
			this.getProject().setNewProperty(allTargets, setToString(allTargetSet));
		}

		if (allTargetsPath != null) {
			this.getProject().addReference(allTargetsPath, setToPath(allTargetSet));
		}

		if (outputSources != null) {
			this.getProject().setNewProperty(outputSources, setToString(sourceSet));
		}

		if (outputTargets != null) {
			this.getProject().setNewProperty(outputTargets, setToString(targetSet));
		}

		if (outputSourcesPath != null) {
			this.getProject().addReference(outputSourcesPath, setToPath(sourceSet));
		}

		if (outputTargetsPath != null) {
			this.getProject().addReference(outputTargetsPath, setToPath(targetSet));
		}
	...
	/**
	 * Sets property to true and/or executes embedded do if any of the target
	 * file(s) do not have a more recent timestamp than (each of) the source
	 * file(s).
	 */
	public void execute() {
		if (!eval()) {
	...
	private boolean outOfDate(File sourceFile, File targetFile) {
		...
		if (ret) {
			if ((sourceFile != null && sourceSet.get(sourceFile) == null) || targetSet.get(targetFile) == null) {
				log("SourceFile " + sourceFile + " outofdate " + "with regard to " + targetFile, verbosity);
			}
			if (sourceFile != null) {
	...
	public class DeleteTargets {
		...
		/**
		 * whether to delete all the targets or just those that are newer than
		...
		/**
		 * Attempt to fix possible race condition when deleting files on WinXP.
		 * If the delete does not work, wait a little and try again.
		 */
