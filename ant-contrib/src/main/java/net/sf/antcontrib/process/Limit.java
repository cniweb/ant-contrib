...
public class Limit extends Task implements TaskContainer {
	...
	/**
	 * How long to wait for all nested tasks to complete, in units. Default is
	 * to wait 3 minutes.
	 *
	 * @param wait
	...
	public void execute() throws BuildException {
		try {
			...
			if (runner.isAlive()) {
				...
				msg.append("Interrupted task <").append(currentTask.getTaskName()).append(">. Waited ")
						.append((maxwait)).append(" ").append(unit.getValue())
						.append(", but this task did not complete.").append((not_ran.length() > 0
