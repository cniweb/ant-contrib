 ...
 * accept a filename containing script input (bash, perl, csh, tcsh, etc.). see
 * antcontrib doc for useage
 *
 * @author stephan beal
 * @author peter reilly
 */
...
public class ShellScriptTask extends ExecTask {
	...
	/**
	 * Disallow the command attribute of parent class ExecTask. ant.attribute
	 * ignore="true"
	 * 
	 * @param notUsed
	 *            not used
	...
	/**
	 * Sets the suffix for the tmp file used to contain the script. This is
	 * useful for cmd.exe as one can use cmd /c call x.bat
	 * 
	 * @param tmpSuffix
	 *            the suffix to use
	 */
	...
	public void execute() throws BuildException {
		...
		/*
		 * if (shell == null) { // Get the default shell shell =
		 * Platform.getDefaultShell();
		 * 
		 * // Get the default shell arguments String args[] =
		 * Platform.getDefaultShellArguments(); for (int
		 * i=args.length-1;i>=0;i--)
		 * this.cmdl.createArgument(true).setValue(args[i]);
		 * 
		 * // Get the default script suffix if (tmpSuffix == null) tmpSuffix =
		 * Platform.getDefaultScriptSuffix();
		 * 
		 * }
		 */
		if (shell == null)
		...
		try {
			/*
			 * // The following may be used when ant 1.6 is used. if (tmpSuffix
			 * == null) super.setInputString(script.toString()); else
			 */
			{
				writeScript();
			...
			if (tmpFile != null) {
				if (!tmpFile.delete()) {
					log("Non-fatal error: could not delete temporary file " + tmpFile.getAbsolutePath());
				}
			}
		}
	}
